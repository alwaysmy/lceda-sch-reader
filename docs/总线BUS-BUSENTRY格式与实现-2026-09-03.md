...
- 多段 `A[2:3]B[7:6]`：order 0/1/2/3 → `A2B7/A2B6/A3B7/A3B6`
  （最后一段最快、各段按书写顺序，含降序）。
  **2026-09-09 已用 CDP 自建多段样本实测确认**：引擎对 4 条分支自动
  命名为 A2B7/A2B6/A3B7/A3B6（order 0/1/2/3），与 `expand_bus_net`
  完全一致。样本存 `examples/CDP探针-多段总线.eprj2`，
  回归见 `probes/verify_bus.py` [4]。
- 越界/无段名返回 None（不猜）。
