"""对比 compressFull 输出与 DB blob 形态。"""
import io, sys, json
sys.path.insert(0, r"D:\WorkDesigns\2_WorkProjects\E_distance\6_tools\lceda_sch_reader\probes")
import importlib.util
spec = importlib.util.spec_from_file_location(
    "cdp", r"D:\WorkDesigns\2_WorkProjects\E_distance\6_tools\lceda_sch_reader\probes\cdp_eval.py")
cdp = importlib.util.module_from_spec(spec)
sys.argv = ["cdp"]
spec.loader.exec_module(cdp)

import urllib.request
targets = json.loads(urllib.request.urlopen(
    "http://127.0.0.1:9222/json/list", timeout=5).read().decode())
t = next(x for x in targets if x["type"] == "page")
ws = cdp.WS(t["webSocketDebuggerUrl"])
ws.send_json({"id": 0, "method": "Runtime.enable"})
try:
    ws.recv_text()
except SystemExit:
    pass

def ev_await(expr, tid=2, timeout=300):
    ws.send_json({"id": tid, "method": "Runtime.evaluate",
                  "params": {"expression": expr, "returnByValue": True,
                             "awaitPromise": True}})
    import time
    t0 = time.time()
    while time.time() - t0 < timeout:
        msg = json.loads(ws.recv_text())
        if msg.get("id") == tid:
            r = msg.get("result", {})
            if "exceptionDetails" in r:
                print("EXC:", json.dumps(r["exceptionDetails"],
                                         ensure_ascii=False)[:400])
                return None
            return r.get("result", {}).get("value")
    raise SystemExit("超时")

r = ev_await("""
(async function(){
  var su = Object.keys(SCH.gVars.projectMgr.sheetCache.sheet)[0];
  var dm = await SCH.docMemoryManager.getOrInitDoc(su);
  var out = {su: su};
  // 找 encodeConsistency / compressFull 持有者
  var cand = [];
  var scan = function(obj, name, depth){
    if (!obj || depth > 2) return;
    try {
      Object.keys(obj).forEach(function(k){
        try{
          var v = obj[k];
          if (v && typeof v === 'object' &&
              typeof v.compressFull === 'function') {
            cand.push(name + '.' + k);
          }
        }catch(e){}
      });
    } catch(e){}
  };
  scan(dm, 'dm', 0);
  scan(dm.consistencyImpl, 'ci', 1);
  if (dm.consistencyImpl) { scan(dm.consistencyImpl.handler, 'ci.handler', 2); }
  out.candidates = cand;
  // 尝试调用
  for (var i = 0; i < cand.length; i++) {
    try {
      var parts = cand[i].split('.');
      var o = dm;
      var f;
      for (var j = 0; j < parts.length; j++) {
        o = (j===0)? dm : o[parts[j-1]];
        f = o[parts[j]];
      }
      var r2 = f.compressFull({});
      if (r2 && r2.then) r2 = await r2;
      out['cf_' + cand[i]] = {
        type: typeof r2,
        len: r2 ? String(r2).length : 0,
        head: r2 ? String(r2).slice(0, 80) : null};
    } catch(e) {
      out['cf_' + cand[i]] = 'ERR ' + String(e).slice(0,80);
    }
  }
  return out;
})()
""")
print(json.dumps(r, ensure_ascii=False, indent=1)[:1800])
