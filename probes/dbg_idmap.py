"""handler.getSourceCodeIdMap + _state 探查。"""
import io, sys, json
...
(async function(){
  ...
  var out = {};
  try { out.idMap = h.getSourceCodeIdMap(); } catch(e){ out.idMapErr = String(e); }
  try {
    out.stateKeys = h._state ? Object.keys(h._state).slice(0,15) : null;
    if (h._state) {
      out.stateSample = JSON.stringify(h._state).slice(0, 400);
    }
  } catch(e){ out.stateErr = String(e); }
  out.sourceCodeIdByType = h.setSourceCodeIdByType ?
    h.getSourceCodeIdMap() : null;
  return out;
})()
""")
