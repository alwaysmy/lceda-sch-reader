"""consistencyImpl 序列化能力探查 + shapeManager 结构。"""
import io, sys, json
...
(async function(){
  ...
  var out = {ciType: typeof ci};
  if (ci) {
    out.ciOwn = Object.keys(ci).slice(0,20);
    var fns = [];
    var o = ci, n = 0;
    while(o && n < 3){
      Object.getOwnPropertyNames(o).forEach(function(p){
        try{ if(typeof ci[p]==='function') fns.push(p); }catch(e){}
      });
      o = Object.getPrototypeOf(o); n++;
    }
    out.ciFns = Array.from(new Set(fns)).filter(function(p){
      return /data|compress|full|save|export|str/i.test(p);});
    out.ciAllFns = Array.from(new Set(fns)).slice(0,50);
  }
  out.smKeys = Object.keys(dm.shapeManager||{}).slice(0,25);
  return out;
})()
""")
